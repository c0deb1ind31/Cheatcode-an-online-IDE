...
def makeSubmission(request, pid):
    ...
    submission = Submissions(user_id=data["user_id"], problem_id=pid, code_lang=data["lang"], submission_code=data["code"],
                            submission_type=data["submission_type"], test_input=data["test_input"], status="pending")
    submission.save()
    run_submission.delay(submission.id,submission.submission_code,submission.test_input,submission.code_lang)
